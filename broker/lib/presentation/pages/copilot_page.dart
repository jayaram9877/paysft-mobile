...
/// Broker Copilot — an on-device AI assistant that answers from the broker's
/// live data (projects, alignments, leads, clients). No cloud, no LLM.
...
class _CopilotPageState extends State<CopilotPage> {
  ...
  Future<void> _submit([String? preset]) async {
    ...
    await context
        ...
        .send(text, context.read<ProjectsProvider>());
    _scrollToBottom();
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (message.projects.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(left: 40, top: 8),
              child: Column(
                children: message.projects
                    .map((p) => _ProjectResultCard(project: p))
                    .toList(),
              ),
            ),
        ],
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.of(context).push(
        MaterialPageRoute(builder: (_) => ProjectDetailPage(seed: project)),
      ),
      child: Container(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    [project.typeLabel, project.location.isEmpty ? null : project.location]
                        .whereType<String>()
                        .join(' · '),
                    style: const TextStyle(
