...
Makes `https://buyer.demo.paysft.com/projects/{id}` open the buyer app when
...
- Android: `<intent-filter android:autoVerify="true">` for
  `https://buyer.demo.paysft.com/projects/*` in `AndroidManifest.xml`.
- iOS: `ios/Runner/Runner.entitlements` with
  `applinks:buyer.demo.paysft.com`.
...
`https://buyer.demo.paysft.com/.well-known/assetlinks.json`
...
`https://buyer.demo.paysft.com/.well-known/apple-app-site-association`
